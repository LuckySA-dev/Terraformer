...
5. Approve the manual-add result, then request facts, interfaces, and a running
...
The phase 1 node view may show sanitized facts, interface operational state,
snapshot metadata, and an event timeline. “Observed” means read from a device;
it does not guarantee freshness. Refresh deliberately and avoid polling fragile
...
- CIDR discovery, CDP/LLDP traversal, and topology canvas;
